...
class StatisticsDashboard():
    app = dash.Dash()
    app.callback(dash.dependencies.Output('linear_graph', 'figure'),
    ...
    def update_graph(self, xaxis_column_name, yaxis_column_name,):
        print(1)
        fig = px.scatter(self.df, x=xaxis_column_name, y=yaxis_column_name)
    ...
    def _generate_table(self, max_rows=10):
        df = self.settings['data'].describe(include=self.settings['metrics']).reset_index()
        return html.Table([
            html.Thead(
    ...
    def _generate_linear(self):
        available_indicators = self.df.columns.unique()
        return html.Div([
                        html.Div([
                            html.Div([
                                dcc.Dropdown(
