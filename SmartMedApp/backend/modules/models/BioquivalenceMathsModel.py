...
class BioquivalenceMathsModel:
    ...
    def get_oneside_eq(self, x: np.array, y: np.array, df: pd.DataFrame) -> tuple:
        ...
        left = float(np.mean(x) - np.mean(y) - dft*(4*df.iloc[1, 2]/(x.size + y.size))**(1/2))
        right = float(np.mean(x) - np.mean(y) + dft*(4*df.iloc[1, 2]/(x.size + y.size))**(1/2))
        return left, right
    ...
    def two_factor_anova(self, t_1: np.array, r_1: np.array, t_2: np.array, r_2:np.array) -> tuple:
        n = 4 * len(t_1)
    ...
    def get_crossover_oneside_eq(self, x: np.array, y: np.array, df: pd.DataFrame) -> tuple:
        left = float(np.mean(x) - np.mean(y) - stats.t.ppf(1-self.alpha, df.iloc[3, 1])*(2*df.iloc[3, 2]/(x.size + y.size))**(1/2))
        right = float(np.mean(x) - np.mean(y) + stats.t.ppf(1-self.alpha, df.iloc[3, 1])*(2*df.iloc[3, 2]/(x.size + y.size))**(1/2))
        return left, right

    def get_crossover_oneside_noteq(self, x: np.array, y: np.array, df: pd.DataFrame) -> tuple:
        left = float(np.mean(x) - np.mean(y) - stats.t.ppf(1-self.alpha / 2, df.iloc[3, 1])*(2*df.iloc[3, 2]/(x.size + y.size))**(1/2))
        right = float(np.mean(x) - np.mean(y) + stats.t.ppf(1-self.alpha / 2, df.iloc[3, 1])*(2*df.iloc[3, 2]/(x.size + y.size))**(1/2))
        return left, right
    ...
    def run_bio_model(self):
        if self.plan == 'parallel':
            ...
            self.oneside_noteq = self.get_oneside_noteq(self.auc_t, self.auc_r, self.anova[0])
        else:
            ...
            self.anova = self.two_factor_anova(self.auc_t_1, self.auc_r_1, self.auc_t_2, self.auc_r_2)
