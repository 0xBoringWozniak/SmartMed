...
class StatisticsDashboard(Dashboard):
        ...
        if len(df.columns) <= 5:
            return html.Div([html.Div(html.H1(children='Описательная таблица'), style={'text-align': 'center'}),
                             html.Div([
                                 ...
                                 html.Div(dcc.Markdown(children=markdown_text_table), style={
                                          'width': len_text, 'float': 'right', 'display': 'inline-block'})
                             ])
                             ], style={'margin': '50px'}
                            )
        else:
        ...
        return html.Div([html.Div(html.H1(children='Линейный график'), style={'text-align': 'center'}),
                         html.Div([
                             html.Div([
                                 html.Div([
                                     dcc.Markdown(
                                         children="Выберите показатель для оси ОХ:"),
                                     dcc.Dropdown(
            ...
            return html.Div([html.Div(html.H1(children='Таблица корреляций'), style={'text-align': 'center'}),
                             html.Div([
                                 html.Div([
                                     ...
                                     style={'width': len_t, 'display': 'inline-block'}),
                                 html.Div(dcc.Markdown(children=markdown_text_corr), style={
                                          'width': len_text, 'float': 'right', 'display': 'inline-block'})
                             ])
                             ], style={'margin': '50px'}
                            )
    ...
    def _generate_hist(self):
        ...
        available_indicators = self.pp.get_numeric_df(
            self.settings['data']).columns.unique()
            ...
            return html.Div([html.Div(html.H1(children='Круговая диаграмма'), style={'text-align': 'center'}),
                             html.Div([
                                 html.Div([
                                     html.Div([
                                         dcc.Markdown(
                                             children="Выберите показатель для оси ОX:"),
                                         dcc.Dropdown(
    ...
    def _generate_dotplot(self):
        ...
        self.app.callback(dash.dependencies.Output('Dot Plot', 'figure'),
                          dash.dependencies.Input(
                              'xaxis_column_name_dotplot', 'value'),
