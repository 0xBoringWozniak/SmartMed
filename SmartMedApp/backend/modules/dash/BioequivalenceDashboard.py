...
class BioequivalenceDashboard(Dashboard):
    ...
    def _generate_param(self):
        ...
        df = pd.DataFrame(data)
        df = round_df(df)
        return html.Div([html.Div(html.H1(children='Таблица с распределением ключевых параметров по группам'), style={'text-align':'center'}),
            html.Div([
            html.Div([
                html.Div([dash_table.DataTable(
                    id='param',
                ...
                html.Div(dcc.Markdown(children=markdown_text_param), style={'width': '28%', 'float': 'right', 'display': 'inline-block',
    ...
    def _generate_log_auc(self):
        ...
        df = pd.DataFrame(data)
        df = round_df(df)
        return html.Div([html.Div(html.H1(children='Средние площади под графиком по каждому препарату'), style={'text-align':'center'}),
            html.Div([
            html.Div([
                html.Div([dash_table.DataTable(
                    id='param',
    ...
    def _generate_concentration_time(self, ref=True):
        if ref:
            ...
            def update_graph(yaxis_column_name_conc_r):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_r]))
            ...
            def update_graph(yaxis_column_name_conc_t):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_t]))
                fig.update_xaxes(title='Время')
    ...
    def _generate_concentration_time_log(self, ref=True):
        if ref:
            ...
            def update_graph(yaxis_column_name_conc_r_log):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_r_log]))
                fig.update_xaxes(title='Время')
            ...
            def update_graph(yaxis_column_name_conc_t_log):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_t_log]))
                fig.update_xaxes(title='Время')
    ...
    def _generate_concentration_time_linlog(self, ref=True):
        if ref:
            ...
            def update_graph(yaxis_column_name_conc_r_linlog, yaxis_type_conc_r_linlog):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_r_linlog]))
                fig.update_xaxes(title='Время')
            ...
            def update_graph(yaxis_column_name_conc_t_linlog, yaxis_type_conc_t_linlog):
                fig = go.Figure()
                fig.add_trace(go.Scatter(x = time, y = df.loc[yaxis_column_name_conc_t_linlog]))
                fig.update_xaxes(title='Время')
    ...
    def _generate_concentration_time_cross(self, tr=True):
        if tr:
            ...
            return html.Div([html.Div(html.H1(children='Индивидуальные графики концентрации для пациентов группа TR'), style={'text-align':'center'}),
                html.Div([
                html.Div([
                    html.Div([
                        dcc.Markdown(children="Выберите показатель для оси ОY:"),
            ...
            return html.Div([html.Div(html.H1(children='Индивидуальные графики концентрации для пациентов группа RT'), style={'text-align':'center'}),
                html.Div([
                html.Div([
                    html.Div([
                        dcc.Markdown(children="Выберите показатель для оси ОY:"),
    ...
    def _generate_group_mean(self, tr=True):
        if tr:
            ...
            return html.Div([html.Div(html.H1(children='Средняя концентрация от времени группа TR'), style={'text-align':'center'}),
                html.Div([
                    html.Div([dcc.RadioItems(
            ...
            return html.Div([html.Div(html.H1(children='Средняя концентрация от времени группа RT'), style={'text-align':'center'}),
                html.Div([
                    html.Div([dcc.RadioItems(
    ...
    def _generate_drug_mean(self):
        ...
        return html.Div([html.Div(html.H1(children='Обобщенные данные по двум препаратам'), style={'text-align':'center'}),
